...
def get_user_datasets():
    ...
    while True:
        ...
        if user_input == "ALL": 
            return ATTACK_TYPES, "model_allsets"
        ...
        if bad:
            ...
            continue
        if len(sets) == 1: 
            base = f"model_{sets[0]}"
        else:              
            base = f"model_{len(sets)}sets"
        return sets, base
...
def finalize_plot(model_dir, filename, message_prefix=None):
    ...
    plt.show()
    if message_prefix: 
        print(f"{message_prefix} saved to {path}")
...
if __name__ == "__main__":
    ...
    if MODEL_TYPE == "RF":
        ...
        if (RAND_FLAG): 
            ...
            model = RandomForestClassifier( n_estimators=200, class_weight="balanced", random_state=SEED_VAL, n_jobs=-1)
    elif MODEL_TYPE == "LR":
        # Logistic Regression model
        if (RAND_FLAG): 
            model = LogisticRegression( max_iter=1000, class_weight="balanced")
        else: 
            model = LogisticRegression( max_iter=1000, class_weight="balanced", random_state=SEED_VAL)
    elif MODEL_TYPE == "NN":
        # Neural Network Model
        if (RAND_FLAG): 
            model = MLPClassifier( hidden_layer_sizes=(128,64,32), activation='relu', solver='adam', max_iter=300, learning_rate='adaptive')
        else: 
            model = MLPClassifier(hidden_layer_sizes=(128,64,32), activation='relu', solver='adam', max_iter=300, learning_rate='adaptive', random_state=SEED_VAL)
    ...
    if feature_importance_dict is not None:
        print("\n\nFeature Importances:")
        for name, score in feature_importance_dict.items(): 
